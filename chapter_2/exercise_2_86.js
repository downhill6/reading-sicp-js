...
function math_sqrt(x) {
  if (is_number(x)) {
    ...
    return apply_generic('math_sqrt', list(x));
  }
}
function math_exp(x) {
  if (is_number(x)) {
    ...
    return apply_generic('math_exp', list(x));
  }
}
function math_atan(x, y) {
  if (is_number(x) && is_number(y)) {
    return Math.atan(x, y);
  } else {
    return apply_generic('math_atan', list(x, y));
  }
}
function math_sin(x) {
  if (is_number(x)) {
    ...
    return apply_generic('math_sin', list(x));
  }
}
function math_cos(x) {
  if (is_number(x)) {
    ...
    return apply_generic('math_cos', list(x));
  }
}
...
function install_complex_package() {
  ...
  function mul_complex(z1, z2) {
    return make_from_mag_ang(mul(magnitude(z1), magnitude(z2)), add(angle(z1), angle(z2)));
  }
  function div_complex(z1, z2) {
    return make_from_mag_ang(div(magnitude(z1), magnitude(z2)), sub(angle(z1), angle(z2)));
  }
...
function install_javascript_number_package() {
  ...
  put('math_sqrt', list('javascript_number'), x => tag(Math.sqrt(x)));
  put('math_atan', list('javascript_number', 'javascript_number'), (x, y) => tag(Math.atan(x, y)));
  put('math_sin', list('javascript_number'), x => tag(Math.sin(x)));
  put('math_cos', list('javascript_number'), x => tag(Math.cos(x)));
  put('math_exp', list('javascript_number'), x => tag(Math.exp(x)));
...
function install_rational_package() {
  ...
  put('math_sqrt', list('rational'), x => tag(project(math_sqrt(raise(x)))));
  put('math_atan', list('rational', 'rational'), (x, y) =>
    tag(project(math_atan(raise(x), raise(y)))),
  );
  put('math_sin', list('rational'), x => tag(project(math_sin(raise(x)))));
  put('math_cos', list('rational'), x => tag(project(math_cos(raise(x)))));
  put('math_exp', list('rational'), x => tag(project(math_exp(raise(x)))));
  return 'done';
}
...
function rational_to_number(x) {
  if (type_tag(x) === 'rational') {
    const value = contents(x);
...
add(
  make_complex_from_real_imag(make_javascript_number(2), make_rational(1, 2)),
  make_complex_from_real_imag(make_rational(1, 2), -0.5),
); // 2.5
