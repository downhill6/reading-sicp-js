...
function can_coerce_to(type_tags, target_type) {
  return accumulate(
    (type_tag, result) =>
      result && (type_tag === target_type || !is_undefined(get_coercion(type_tag, target_type))),
    true,
    type_tags,
  );
}
...
function coerce_all(args, target_type) {
  return map(
    arg => (type_tag(arg) === target_type ? arg : get_coercion(type_tag(arg), target_type)(arg)),
    args,
  );
}
...
// 假设现在有三种类型， A，B，C
// 其中 A -> B, B -> C
// 理论上，A 可以经过中间类型 B 转换为 C
// 但现在无法处理这种情况

function install_coercion_package() {
  function rational_to_javascript_number(n) {
    const x = contents(n);
    return make_javascript_number(head(x) / tail(x));
  }
  put_coercion('rational', 'javascript_number', rational_to_javascript_number);
}
install_coercion_package();

add(make_rational(1, 2), make_javascript_number(1)); // 1.5
