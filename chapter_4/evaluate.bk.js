...
function apply(fun, args) {
  if (is_primitive_function(fun)) {
    ...
    const result = evaluate(body, extend_environment(names, values, function_environment(fun)));
...
function list_of_values(exps, env) {
  if (no_operands(exps)) {
    ...
    return pair(evaluate(first_operand(exps), env), list_of_values(rest_operands(exps), env));
  }
}
...
function extend_environment(names, vals, base_env) {
  if (length(names) === length(vals)) {
    ...
    error('Too many arguments supplied: ' + stringify(names) + ', ' + stringify(vals));
  } else {
    error('Too few arguments supplied: ' + stringify(names) + ', ' + stringify(vals));
  }
}
function lookup_name_value(name, env) {
  function env_loop(env) {
    ...
    if (is_empty_environment(env)) {
      ...
      return value === no_value_yet ? error(name, 'Name use before declaration: ') : value;
    }
  }
  return env_loop(env);
}
...
function apply_primitive_function(fun, argument_list) {
  return apply_in_underlying_javascript(primitive_implementation(fun), argument_list);
}
function setup_environment() {
  ...
  const primitive_constant_values = map(f => head(tail(f)), primitive_constants);
...
extend_environment(primitive_function_names, primitive_function_values, the_empty_environment);
const program = parse('const x = 1; let y = 2;x+y;');
