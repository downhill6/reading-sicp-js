...
(function (root, mod) {
  ...
  var isEcma6Keyword = makePredicate(
    ecma5AndLessKeywords + ' let const class extends export import yield',
  );
  ...
  function finishToken(type, val) {
    ...
    if (type === _parenR || type === _braceR) {
      ...
      var statementParens =
        prevType === _if || prevType === _for || prevType === _with || prevType === _while;
  ...
  function parseDoStatement(node) {
    ...
    if (options.ecmaVersion >= 6) eat(_semi);
    else semicolon();
  ...
  function parseForStatement(node) {
    ...
    if (tokType === _var || tokType === _let) {
      ...
      if (
        (tokType === _in || (options.ecmaVersion >= 6 && isContextual('of'))) &&
  ...
  function parseExprList(close, allowTrailingComma, allowEmpty, refShorthandDefaultPos) {
    ...
    while (!eat(close)) {
      ...
      if (allowEmpty && tokType === _comma) {
        ...
        if (tokType === _ellipsis) elts.push(parseSpread(refShorthandDefaultPos));
        else elts.push(parseMaybeAssign(false, refShorthandDefaultPos));
      }
    }
    return elts;
  }
